...
class JsonUDP(object):
    """encapsulate communication with the edge"""

    def __init__(self, address="127.0.0.1", port=5645):
...
class HPYServerManager(object):
    def __init__(self, server_address="127.0.0.1", server_port=5645):
        self.server_address = server_address
        self.rpc_manager = JsonUDP(server_address, server_port)

    """
    Return: {'keep_running': 1}
    """
    def stop(self):
        return self.rpc_manager.read('stop')
    def get_edges(self):
        """
        :return:
            [
                {
                    'community': 'VIP1xxxx',
                    'ip4addr': '10.202.48.190/24',
                    'purgeable': 0,
                    'macaddr': '0A:38:9E:45:0F:C9',
                    'sockaddr': '140.75.170.16:19725',
                    'proto': 'UDP',
                    'desc': 'Openwrt',
                    'last_seen': 1707721121
                },
                {
                    'community': 'VIP0xxxx',
                    'ip4addr': '10.174.143.101/24',
                    'purgeable': 0,
                    'macaddr': '46:B4:81:80:E8:74',
                    'sockaddr': '66.42.100.58:15644',
                    'proto': 'UDP',
                    'desc': 'xxxme',
                    'last_seen': 1707721105
                }
                ...
            ]

        """
        rows = self.rpc_manager.read('edges')
        return rows
    def get_supernodes(self):
        ...
        """
        rows = self.rpc_manager.read('supernodes')
        return rows
...
def main():
    ...
    ap.add_argument('-t', '--mgmtport', action='store', default=5645,
                    help='Management Port (default=5645)', type=int)
    ...
    rpc = JsonUDP(port=args.mgmtport)
